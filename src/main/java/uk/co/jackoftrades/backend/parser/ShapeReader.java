...
import uk.co.jackoftrades.backend.parser.shape.ShapeLexer;
import uk.co.jackoftrades.backend.parser.shape.ShapeParser;
 ...
 * @author ClaudeCode
 */
public class ShapeReader implements Parser<PlayerShape> {
    /**
    ...
    public @NotNull List<PlayerShape> parse(@NotNull String filename) throws IOException {
        try {
            ...
            ShapeLexer lexer = new ShapeLexer(stream);
            CommonTokenStream tokens = new CommonTokenStream(lexer);
            ShapeParser parser = new ShapeParser(tokens);
            ShapeParser.FileContext output = parser.file();
