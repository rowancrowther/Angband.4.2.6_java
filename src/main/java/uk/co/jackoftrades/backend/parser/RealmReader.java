...
import uk.co.jackoftrades.backend.parser.realm.RealmLexer;
import uk.co.jackoftrades.backend.parser.realm.RealmParser;
 ...
 * @author ClaudeCode
 */
public class RealmReader implements Parser<MagicRealm> {
    /**
    ...
    public @NotNull List<MagicRealm> parse(@NotNull String filename) throws IOException {
        try {
            ...
            RealmLexer lexer = new RealmLexer(stream);
            CommonTokenStream tokens = new CommonTokenStream(lexer);
            RealmParser parser = new RealmParser(tokens);
            RealmParser.FileContext output = parser.file();

            return output.realms;
        } catch (Exception e) {
