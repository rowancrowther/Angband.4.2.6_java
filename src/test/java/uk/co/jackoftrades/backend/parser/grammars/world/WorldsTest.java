...
/**
 * Unit tests for the generated {@link Worlds} parser (grammar-track step 8, parser half; paired
 * with {@link WorldsLexer}).
 *
...
class WorldsTest {
    ...
    /**
     * Builds a {@link Worlds} parser over {@code src}, routing errors to {@code errors}.
     */
    private Worlds parser(String src, Errors errors) {
        WorldsLexer lexer = new WorldsLexer(CharStreams.fromString(src));
        Worlds parser = new Worlds(new CommonTokenStream(lexer));
    ...
    void singleRecordExtractsAllFiveFieldsVerbatim() {
        ...
        Worlds.FileContext ctx = parser("record-count:1\nlevel:0:Town:None:Angband 1\n", errors).file();
